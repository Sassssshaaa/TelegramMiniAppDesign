...
import { useimport { sendTelegramOrder } from './telegram';
import { supabase } from './supabase';
...
declare global {
  interface Window {
    ...
    sendTelegramOrder?: (cart: CartItem[], total: number, details: { name: string; phone: string; address: string; payment: string }) => void;
  }
}
...
function HomeScreen({
  ...
  // Исправленный алгоритм фильтрации для главной страницы
  const displayed = products.filter((p) => {
    ...
    // 3. По умолчанию на главной: смотрим флаги "Хит продаж"
    const isHotProduct = p.isHot || p.ishot || p.is_hot;
    // Если флаги ни у одного товара не проставлены, отображаем все
...
function SuccessScreen({ onHome }: { onHome: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: "0 28px", textAlign: "center" }}>
      ...
      <button className="btn-primary" style={{ width: "100%", padding: "16px 0" }} onClick={onHome}>Вернуться в магазин</button>
    </div>
  );
}
